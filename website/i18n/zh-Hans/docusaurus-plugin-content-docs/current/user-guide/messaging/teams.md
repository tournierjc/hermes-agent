...
适配器会下载入站文件附件（不仅是图片）并缓存到本地；带提及门控的消息在下载附件之前就会被丢弃。个人聊天中的出站文件走 Teams 的文件同意卡片（用户点击接受后上传到 OneDrive）；频道/群聊回退为 Bot Framework 附件。机器人可以对消息添加/移除表情反应（处理中 👀，成功 ✅，失败 ❌），并接收用户的 `messageReaction` 事件。流式逐字编辑尚未接入。
